//file:/mnt/db/Dropbox/projects/crypto/mp/GrammarHelperServer/public_html/jquery.js
...
//file:/mnt/db/Dropbox/projects/crypto/mp/testingFramework/PromiseHelperV4.js
...
/*
    ...
    function PromiseHelperV4() {
        var p = PromiseHelperV4.prototype;
        p = this;
        var self = this;
        self.settings = {};
        self.data = {}
        self.data.methods = []
        self.data.methodCounts = 0

        p.init = function initPromiseHelperV4(config={}) {
            self.settings = config, {}
        }

        p.method = function method(a) {
        }
        p.add = function add(fxLink) {
            self.data.methods.push(fxLink)
        }
        p.start = function start(fxLink) {
            self.nextLink()
        }
        p.nextLink = function nextLink() {
            let fx = self.data.methods.shift()
            if (fx == null) {
                self.end()
                return
            }
            //fx( self.nextLink)
            fx(function on() {
                setTimeout(self.nextLink)
            })
            self.data.methodCounts++
            // console.log('finished', self.data.methodCounts)
        }

        p.end = function end() {
            console.log('finished', self.data.methodCounts, 'methods')
        }

        p.testPromiseHelperV4 = function testPromiseHelperV4(config) {
    ...
    window.PromiseHelperV4 = PromiseHelperV4

    PromiseHelperV4.pAll = function pAll() {
        let args = sh.args(arguments)

        let h = {}
        h.e = new Error()

        //let w = PromiseHelperV3.qPT()

        let w = new PromiseHelperV4();
        var config = {};
        w.init(config)
        setTimeout(function () {
            w.start()
        ...
        sh.each(args, function onEach(k, fx) {
            if (fx == 'break') {
                return false
            }
            if (fx == null) {
                sh.throw('promise.callbackFx index at ' + (k + 1) + ' is not a valid function')
            }
            w.add(function on(cb, t) {
                if (sh.isFunction(fx) == false) {
                    console.error('cannot add fx arg for pAll', (k + 1))
                    console.error(h.e.stack)
                    sh.throw('this is bad on item:' + (k + 1), fx)
                    return;
                }
                try {
                    sh.jw(function (onTunItem) {
                        if (PromiseHelperV3.alt != true) {
                            fx(cb, w)
                        } else {
                            fx(w, cb)
                        }
                    ...
                    let e2 = e.stack//()
                    console.error(e)
                    // QuickDialog.notificationMessage(e2, 'failed this test')
                }
            })
        })
        ...
        // w.addSync(CreditHelper2F.closeCreditHelper)

        return w;
    }

    function fxTest_PromiseHelperV4() {
        let i = new PromiseHelperV4();
        var config = {};
        i.init(config)
        i.testPromiseHelperV4();
    ...
    if ( window.u ){
        if (u.redoing('redo_PromiseHelperV4')) {
            if (window.fxReload_PromiseHelperV4) {
                sh.jw(window.fxReload_PromiseHelperV4)
            } else {
                sh.jw(fxTest_PromiseHelperV4)
            }
        }
...
//file:/mnt/db/Dropbox/projects/crypto/mp/GrammarHelperServer/sharedResourcesGrid/js/quick_base/quickloadv4.js

(function () {

        window.reloader_getAuthRemote = 'localhost:10110/api/fileList?file='
        ...
        window.QuickLoadV4 = true;

        window.dirsRead = {}

        function QuickLoadV4() {
            var p = QuickLoadV4.prototype;
            p = this;
            var self = this;
            ...
            p.addEarlyEntryPoints = function addEarlyEntryPoints() {
            ...
            self.addEarlyEntryPoints()


            p.overrideAjax = function overrideAjax() {
                $.ajax = function overrideAjaxToDirectoToCache() {
...
*/
  ...
  */
            ...
            p.loadAllItems = function loadAllItems() {

                let i = new PromiseHelperV4();
                var config = {};
                i.init(config)
                i.add(self.createDialogQuickNav)
                i.add(self.loadCachedJSONMap)
                i.add(self.loadInitUIUtils)
                i.add(self.setBasicGlobalUrls)
                i.add(self.loadCSS)
                i.add(self.loadSubDirectories)
                i.add(self.loadDirsFile)
                i.add(self.loadFramework)
                i.add(self.fxFiniehdLoading)

                i.add(function onf(cb, t) {
                    console.log('done loading quickloader')
                    cb()
                })
                i.start()
            ...
            p.createDialogQuickNav = function createDialogQuickNav(cb) {
...
`
            ...
            p.loadCachedJSONMap = function loadCachedJSONMap(cb) {
                ...
                window.DlFilesFromDir_DoNotAppend = true

                window.reloader = {}
                window.rr = window.reloader
                window.reloader.ir = function on() {
                }
                window.reloader.reloadBlockFx = function on() {
                }
                ...
                function onGotJ(data) {
                    ...
                    function decreateAndParseCache() {
                        let cfg = {}
                        cfg.strs = window.vsr
                        ...
                        cfg.text = data;
                        cfg.rev = true
                        let x = self.ql_updateSh()
                        let ttt = x.gsubBulk(cfg)
                    ...
                    decreateAndParseCache()

                    //self.data.cache = data;
                    self.data.cache = {}
                    $.each(data, function onEach(k, v) {
                        let key = k
                        if (self.settings.ql_removeProtcols != false) {
            ...
            p.loadInitUIUtils = function loadInitUIUtils(cb) {
                let dirFile = 'http://127.0.0.2:10110/api/fileList?file='
                ...
                var scripts2 = [
                    ...
                    dirFile + '/mnt/db/Dropbox/projects/crypto/mp/GrammarHelperServer/sharedResourcesGrid/js/download_files/DlFilesFromDir.js',
                    dirFile + '/mnt/db/Dropbox/projects/crypto/mp/GrammarHelperServer/sharedResourcesGrid/js/download_files/DlFilesBootLoader.js',
                ...
                self.loadScript2(scripts2.concat(), cb);
            }

            p.setBasicGlobalUrls = function setBasicGlobalUrls(cb) {
            ...
            p.loadSubDirectories = function loadSubDirectories(cb) {
            ...
            p.loadDirsFile = function loadDirsFile(cb) {
                window.redoingDict = {}
                let i = instance = new DlFilesBootLoader();
                var config = {};
                i.init(config)
                i.processFiles(cb);
            }

            p.loadFramework = function loadFramework(cb) {


                let i = new DlQlFramework();
                var config = {};
            ...
            p.fxFiniehdLoading = function fxFiniehdLoading(cb) {
 ...
 */
                ...
                if (window.QuickLoadV4DebugLocal) {
                    debugger
                }
        ...
        window.QuickLoadV4 = QuickLoadV4;
        ...
        if (window.u) {
            if (window.u.redoing) {
                if (u.redoing('redo_QuickLoadV4')) {
                    ...
                    var config = {};
                    let qlConfig = {}
                    qlConfig.reloadCache2 = true;
                    qlConfig.cacheFile = 'http://localhost:10110/api/fileList?file='
                        + ('/home/user1/trash/rcPub/themes/cached/react.templates.min.html')
                    config.qlConfig = qlConfig
